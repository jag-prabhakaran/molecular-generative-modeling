...
app.config["SQLALCHEMY_DATABASE_URI"] = os.getenv("POSTGRES_URL_SECRET")
app.config[
...
def insert_data():
    ...
    if (
        "scaffold_smile" not in data
        or "log_p_min" not in data
        or "log_p_max" not in data
    ):
        ...
        id = hashlib.md5(
            (scaffold_smile + str(log_p_min) + str(log_p_max)).encode("utf-8")
        ).hexdigest()
...
def run_model_inference():
    ...
    if "model_type" not in data:
        return (
            jsonify(
                "Bad Request! Malformed data. Are you missing the model_type parameter?"
            ),
            400,
        )
    ...
    if "payload" not in data:
        return (
            jsonify(
                "Bad Request! Malformed data. Are you missing the payload parameter?"
            ),
            400,
        )
    if data["model_type"] == ModelType.scaffold_constrained:
        ...
        if (
            ...
            return (
                jsonify(
                    "Bad Request! Malformed data. Are you missing the scaffold_smile, log_p_min, or log_p_max parameters in your payload?"
                ),
                400,
            )
        else:
            response = requests.post(
                "http://scaffold_constrained:5000/run_model_inference", json=payload
            )
        ...
        if "log_p_min" not in payload or "log_p_max" not in payload:
            return (
                jsonify(
                    "Bad Request! Malformed data. Are you missing the log_p_min, or log_p_max parameters in your payload?"
                ),
                400,
            )
