...
parser.add_argument(
    "--learning-rate", action="store", dest="learning_rate", type=float, default=0.0005
)
parser.add_argument(
    "--num-steps", action="store", dest="n_steps", type=int, default=3000
)
parser.add_argument(
    "--batch-size", action="store", dest="batch_size", type=int, default=64
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Number of experience sequences to sample each step. "
...
if __name__ == "__main__":
    ...
    if arg_dict["scoring_function_kwargs"]:
        ...
        if not len(kwarg_list) % 2 == 0:
            raise ValueError(
                "Scoring function kwargs must be given as pairs, "
