...
def hill_climbing(
    ...
    logger.log(
        Agent.rnn.gru_2.weight_ih.cpu().data.numpy()[::100],
        "init_weight_GRU_layer_2_w_ih",
    )
    logger.log(
        Agent.rnn.gru_2.weight_hh.cpu().data.numpy()[::100],
        "init_weight_GRU_layer_2_w_hh",
    )
    logger.log(
        Agent.rnn.embedding.weight.cpu().data.numpy()[::30], "init_weight_GRU_embedding"
    )
    logger.log(
        Agent.rnn.gru_2.bias_ih.cpu().data.numpy(), "init_weight_GRU_layer_2_b_ih"
    )
    logger.log(
        Agent.rnn.gru_2.bias_hh.cpu().data.numpy(), "init_weight_GRU_layer_2_b_hh"
    )
    ...
    for step in range(n_steps):
        ...
        logger.log(
            Agent.rnn.embedding.weight.cpu().data.numpy()[::30], "weight_GRU_embedding"
        )
        logger.log(
            Agent.rnn.gru_2.bias_ih.cpu().data.numpy(), "weight_GRU_layer_2_b_ih"
        )
        logger.log(
            Agent.rnn.gru_2.bias_hh.cpu().data.numpy(), "weight_GRU_layer_2_b_hh"
        )
        logger.log(
            "\n".join(
                [
                    smiles + "\t" + str(round(score, 2))
                    for smiles, score in zip(smiles[:12], score[:12])
                ]
            ),
            "SMILES",
            dtype="text",
    ...
    if not save_dir:
        save_dir = "data/results/run_" + time.strftime(
    ...
    with open(os.path.join(save_dir, "memory.smi"), "w") as f:
        for i, exp in enumerate(experience.memory):
            try:
                if (
                    Chem.MolToSmiles(
                        Chem.rdmolops.RemoveStereochemistry(Chem.MolFromSmiles(exp[0]))
                    )
                    not in previous_smiles
                ):
                    f.write("{}\n".format(exp[0]))
