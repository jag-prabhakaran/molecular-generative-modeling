...
def decode_rationales(model, rationale_dataset):
    ...
    with torch.no_grad():
        for init_smiles in tqdm(loader):
            ...
            mols = [
                (x, y) for x, y in zip(init_smiles, final_smiles) if y and "." not in y
            ]
            mols = [
                (x, y)
                for x, y in mols
                if Chem.MolFromSmiles(y).HasSubstructMatch(Chem.MolFromSmiles(x))
            ]
...
def property_filter(cand_mols, scoring_function, args):
    ...
    rationale_dist = {
        r: x / n for r, (x, n) in rationale_dist.items() if x / n >= args.alpha
    }
...
if __name__ == "__main__":
    ...
    with open(args.rationale) as f:
        ...
        rationale_dataset = SubgraphDataset(
            rationales, args.atom_vocab, args.decode_batch_size, args.num_decode
        )
    ...
    for epoch in range(args.load_epoch + 1, args.epoch):
        ...
        for total_step, batch in enumerate(dataset):
            ...
            meters = meters + np.array(
                [kl_div, loss.item(), wacc * 100, tacc * 100, sacc * 100]
            )
