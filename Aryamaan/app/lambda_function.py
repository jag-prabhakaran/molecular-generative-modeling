...
def lambda_handler(event, context):
    ...
    if model_type == 'vae-gan':
        if 'num_molecules' not in payload or 'log_p_min' not in payload or 'log_p_max' not in payload or 'num_molecules' not in payload:
            return json.dumps({
                "error": "Missing parameters in payload."
                })
    ...
    response = runtime.invoke_endpoint(EndpointName=model_type,
